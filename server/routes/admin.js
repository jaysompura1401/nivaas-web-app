...
router.get("/stats", requireAuth, adminOnly, async (_req, res) => {
  try {
    const [[users]]      = await pool.query("SELECT COUNT(*) AS total, SUM(is_verified) AS verified FROM nivaas_users");
    const [[props]]      = await pool.query("SELECT COUNT(*) AS total, SUM(verified) AS verified, SUM(views_count) AS total_views FROM nivaas_properties");
    const [[complaints]] = await pool.query("SELECT COUNT(*) AS total, SUM(status='open') AS open FROM nivaas_complaints");
    const [[visits]]     = await pool.query("SELECT COUNT(*) AS total, SUM(status='pending') AS pending FROM nivaas_property_visits");
    const [[revenue]]    = await pool.query(
      "SELECT SUM(amount) AS total_collected FROM nivaas_rent_payments WHERE status='paid'"
    );
    const [[agreements]] = await pool.query(
      "SELECT COUNT(*) AS total, SUM(status='signed') AS active FROM nivaas_agreements"
    );

    res.json({ users: users[0] ?? users, properties: props, complaints, visits, revenue, agreements });
  } catch (err) {
...
router.get("/users", requireAuth, adminOnly, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, full_name, email, phone, role, is_verified, kyc_status, city, created_at
       FROM nivaas_users ${where}
       ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      params
    );
    const [[{ total }]] = await pool.query(
      `SELECT COUNT(*) AS total FROM nivaas_users ${where.replace("LIMIT ? OFFSET ?","")}`,
      params.slice(0, -2)
    );
...
router.patch("/users/:id", requireAuth, adminOnly, async (req, res) => {
  try {
    const { role, is_verified, kyc_status } = req.body;
    const updates = [], vals = [];
    ...
    if (kyc_status  !== undefined) { updates.push("kyc_status=?");  vals.push(kyc_status); }
...
router.patch("/properties/:id", requireAuth, adminOnly, async (req, res) => {
  try {
    ...
    if (verified            !== undefined) { updates.push("verified=?");            vals.push(verified ? 1 : 0); }
